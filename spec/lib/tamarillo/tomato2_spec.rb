...
describe Tomato2 do
  ...
  subject { Tomato2.new(now, 25.minutes, clock) }
  ...
  describe "comparison" do
    it "is the same if the start-times match" do
      ...
      tomato_a = Tomato2.new(time_a, 25.minutes)
      tomato_b = Tomato2.new(time_b, 25.minutes)
    ...
    it "is not the same if the duration differs" do
      ...
      tomato_a = Tomato2.new(time_a, 15.minutes)
      tomato_b = Tomato2.new(time_b, 25.minutes)
  ...
  describe "approximate time remaining" do
    let(:clock) { stub(now: now + elapsed) }
    subject { Tomato2.new(now, 10.minutes, clock) }
